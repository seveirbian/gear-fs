			     ...
			     struct dentry **ret)
{
	...
	char *base_path;
	char send_buf[500];
	int send_buf_len = 500;
	// struct ovl_entry *oe = NULL;
	memset(finger_print, 0, finger_print_len);
	memset(buf, 0, buf_len);
	// memset(content, 0, count);
	memset(send_buf, 0, send_buf_len);
	base_path = dentry_path_raw(base, buf, buf_len);
	// printk("base path: %s\n", base_path);
	// printk("base_path len: %d\n", strlen(base_path));
	// printk("name: %s\n", name);
	// printk("last_element: %d\n", last_element);
	// printk("is_dir: %d\n", d->is_dir);
	...
	// printk("d_can_lookup: %d\n", d_can_lookup(this));
	// gear
	if (d_is_reg(this)) {
		// 丢弃这个dentry
		dput(this);

		strcpy(send_buf, base_path);
		strcat(send_buf, "/");
		...
		send_path(send_buf, strlen(send_buf));
